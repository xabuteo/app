...
def show():
    ...
    # Format and filter
    df["EVENT_START_DATE"] = pd.to_datetime(df["EVENT_START_DATE"]).dt.strftime('%Y-%m-%d')
    df["EVENT_END_DATE"] = pd.to_datetime(df["EVENT_END_DATE"]).dt.strftime('%Y-%m-%d')
    ...
    # When NO event is selected, show table and filters
    if not selected_event_id:
        ...
        if title_filter:
            df_display = df_display[df_display["EVENT_TITLE"].str.contains(title_filter, case=False, na=False)]
        if type_filter != "All":
            df_display = df_display[df_display["EVENT_TYPE"] == type_filter]
        if status_filter != "All":
            df_display = df_display[df_display["EVENT_STATUS"] == status_filter]

        if df_display.empty:
            st.info("No events match the filter.")
        else:
            selected_row_id = st.radio(
                "Select an Event",
                options=df_display["ID"],
                format_func=lambda x: f"{df_display[df_display['ID'] == x]['EVENT_TITLE'].values[0]} ({df_display[df_display['ID'] == x]['EVENT_START_DATE'].values[0]})"
            )
            if st.button("View Selected Event"):
                st.session_state.selected_event_id = selected_row_id
                st.rerun()

            st.data_editor(df_display, disabled=True, hide_index=True, use_container_width=True)

        new_event.add_new_event()

    # When event is selected, show back button and tabs
    else:
